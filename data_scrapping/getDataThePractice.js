...
export async function fetchYogaClassesThePractice() {
  ...
  const calendarData = await page.$eval(".mec-calendar", (calendar) => {
    ...
    [...days].forEach((day) => {
      const date = moment(day.getAttribute("data-mec-cell")).format("DD/MM/YY");
  ...
  const parsedClasses = calendarData
...
// fetchYogaClassesThePractice().then((classes) => console.log(classes));
